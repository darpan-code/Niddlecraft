...
@section('body')
    ...
    <div class="flex justify-between items-center mt-[6vw] mx-[5vw]">
        ...
        <div class="flex items-center w-[30vw] h-[3vw] bg-[#dee2e6] rounded-full">
            <a href="{{ route("delivery-appointments") }}"><div class="w-[15vw] h-[3vw] text-[1.7vw] font-medium flex items-center justify-center rounded-full bg-[#0056b3] text-white">Appointments</div></a>
    ...
    <div class="flex justify-between items-center text-center mt-[1vw] mx-auto bg-[#0074D6] text-white font-semibold h-[4vw] rounded-[0.5vw] px-[1vw] text-[1.5vw] w-[92vw]">
        ...
        <div class="w-[10vw]">Delivery Date</div>
    ...
    @if ($data)
        @foreach ($AppointmentData as $data)
            <div class="flex text-center justify-between items-center font-medium bg-[#ECECEC] h-[4vw] mx-auto mb-[1vw] px-[1vw] rounded-[0.5vw] text-[1.2vw] w-[92vw]">
                ...
                <div class="w-[10vw]">{{ $data->delivery_date }}</div>
                    ...
                    <div class="modal-content w-[85vw] h-[45vw] relative">
                        ...
                        <div class="text-[1.3vw] w-[64vw] min-h-[15vw] p-[2vw] font-bold flex">
                            ...
                            <div class="min-w-[24vw]">
                                ...
                                <div class="flex"><div class="w-[9vw]">Delivery Date </div><div class="font-semibold ml-[.5vw]">: {{ $data->delivery_date }}</div></div>
                        ...
                        <div class="flex justify-between items-center mt-[2vw]">
                            ...
                            <form action="{{ route('delivery-appointments') }}" method="POST">
                                @csrf
                                <select class="border-[0.1vw] border-black text-[1.2vw] w-[14vw] h-[2vw]" name="status" id="delivery-order-status{{ $data->apid }}">
                                    <option value="" selected>Select</option>
                                    <option value="Delivered">Delivered</option>
                                    <option value="Rejected">Rejected</option>
                                </select>
                                <select class="ml-[2vw] border-[0.1vw] border-black text-[1.2vw] w-[18vw] h-[2vw]" name="reason" id="delivery-order-reason{{ $data->apid }}">
                                    <option value="" selected>Select</option>
                                </select>
            ...
            <script>
                $(document).ready(function () {
                    // Delivery Order Status Option Change
                    $("#delivery-order-status{{ $data->apid }}").change(function () { 
                        var channgeValue = $("#delivery-order-status{{ $data->apid }}").val();
                        
                        switch (channgeValue) {
                            case "Rejected":
                                $("#delivery-order-reason{{ $data->apid }}").html('\
                                <option value="" selected>Select</option>\
                            ...
                            default:
                                $("#delivery-order-reason{{ $data->apid }}").html('<option value="" selected>Select</option>');
                                break;
                        }
                    });
                });
