...
@section('body')
    ...
    <div class="flex justify-between items-center mt-[6vw] mx-[5vw]">
        ...
        <div class="flex items-center w-[30vw] h-[3vw] bg-[#dee2e6] rounded-full">
            <a href="{{ route("new-appointments") }}"><div class="w-[15vw] h-[3vw] text-[1.7vw] font-medium flex items-center justify-center rounded-full bg-[#0056b3] text-white">Appointments</div></a>
    ...
    @if ($data)
        @foreach ($AppointmentData as $data)
            <div class="flex text-center justify-between items-center font-medium bg-[#ECECEC] h-[4vw] mx-auto mb-[1vw] px-[1vw] rounded-[0.5vw] text-[1.2vw] w-[92vw]">
                ...
                <div class="w-[10vw]">
                    <div id="date"> {{ $data->order_date }}</div>
                    <div id="time"> {{ $data->order_time }} </div>
                </div>
                ...
                <div class="modal-dialog modal-dialog-centered">
                    <div class="modal-content w-[85vw] h-[45vw] relative">
                        ...
                        <div class="flex justify-between items-center mt-[2vw]">
                            ...
                            <form action="{{ route('new-appointments') }}" method="POST">
                                @csrf
                                <select class="border-[0.1vw] border-black text-[1.2vw] w-[8vw] h-[2vw]" name="status" id="new-order-status{{ $data->apid }}">
                                    <option value="" selected>Select</option>
                                    <option value="Accepted">Accepted</option>
                                    <option value="Rejected">Rejected</option>
                                </select>
                                <select class="ml-[2vw] border-[0.1vw] border-black text-[1.2vw] w-[18vw] h-[2vw]" name="reason" id="new-order-reason{{ $data->apid }}">
                                    <option value="" selected>Select</option>
                                </select>
        ...
        <script>
            $(document).ready(function () {
                // New Order Status Option Change
                $("#new-order-status{{ $data->apid }}").change(function () { 
                    var channgeValue = $("#new-order-status{{ $data->apid }}").val();
                    
                    switch (channgeValue) {
                        case "Rejected":
                            $("#new-order-reason{{ $data->apid }}").html('\
                            <option value="" selected>Select</option>\
                        ...
                        default:
                            $("#new-order-reason{{ $data->apid }}").html('<option value="" selected>Select</option>');
                            break;
                    }
                });
            });
